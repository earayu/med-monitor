...
def life_label(p):
    """v3: IBI389 gets the adjudicated long label instead of bare 未知."""
    lc = lifecycle_of(p)
    if lc == "未知" and p["display"] == "IBI389":
        return "登记状态未知（项目开发状态待核实）"
    return lc
...
L = []
A = L.append
A("# 实体瘤 TCE 产品进展与最新临床结果 (截至 2026-08-03, 定稿)")
A("")
A("> **数据来源**: ClinicalTrials.gov 试验登记 (一阶段, 52 产品全命中) + PubMed 2023-01-01 以来临床结果类文献 (二阶段, E-utilities 拉取, 疗效数字一律照抄摘要原文)。")
A("> **编制说明**: 会议摘要 (ASCO/ESMO/AACR) 与公司更新未纳入本版。")
A("> **版本**: v1 机械拉取初稿 → v2 按 @徐陆恺 复核意见修订 (产品身份、两轴分类、新增 8 品、疗效语境) → 本版为 @徐陆恺 最终裁定后的定稿 (NCT07258121 归属、IBI389 状态措辞、计数 194/193、VIR-5818 更名等)。")
A("")
A("## 1. 总览统计")
A("")
A(f"- 产品总数: **{n_total}** (v1 47 - 移除 2 - 合并 1 + 新增 8)")
A(f"- 试验登记口径: **{assoc} 个产品-试验关联, {unique_ncts} 个唯一 NCT** (同一 NCT 可关联多个产品)")
A(f"  - 共享 NCT 2 个: NCT04262466 (tebentafusp+brenetafusp 真实联合, 两边保留); NCT07258121 (ct.gov 字段矛盾: title=ZGGS34, intervention 误填 ZG006, 经人工裁定归 ZGGS34, 已从 alveltamig 剔除)")
...
A(f"- 最高阶段分布: {dist}")
A(f"- **{n_with_results}/52 检出经人工保留的 2023+ PubMed 相关文献** (含临床前/综述/非疗效研究, 不均为临床结果; 含明确 ORR 数字: Obrixtamig, IBI389; 联合方案 PR 率: Cibisatamab; PSA50: Pasritamig, Acapatamab)")
A("")
A("### 分子类型 × 生命周期 交叉统计")
A("")
lifes = ["获批", "活动中/在研", "完成", "终止", "撤回", "未知"]
...
sep = "|---|---|---|---|---|---|---|---|"
NAME_OVERRIDE = {"VIR-5818/AMX-818/SAR446368": "VIR-5818（原 AMX-818/SAR446368）"}
RESULT_SUFFIX = {"IBI389": "; 最后登记更新 2023-12-28, 当前招募/开发状态需查公司管线或其他注册库"}
for cat in CAT_ORDER:
    ...
    A(hdr); A(sep)
    for p in ps:
        res = SHORT.get(p["display"], NOLIT) + RESULT_SUFFIX.get(p["display"], "")
        name = NAME_OVERRIDE.get(p["display"], p["query"])
        A(f'| {name} | {p["target"]} | {p["company"]} | {phase_of(p) or "-"} | {life_label(p)} | {status_of(p)} | {res} | {ncts_of(p)} |')
    A("")
A("> 注: 「关键 NCT」优先列在研/有结果的试验; 无在研试验的产品列出全部登记 NCT。登记状态的 UNKNOWN 为 ct.gov 长期未更新标记。")
A("")
...
for p in products:
    ...
    t = trials.get(p["display"], {})
    A(f'| {NAME_OVERRIDE.get(p["display"], p["query"])} | {t.get("trial_count","-")} | {s} |')
A("")
A("### 6.2 v2 移出产品")
A("")
...
A("")
A("---")
A("*定稿: 已按 @徐陆恺 最终裁定修订 (v1 初稿 → v2 复核修订 → v3 裁定定稿)。数字均照抄 ct.gov 登记与 PubMed 摘要原文; 会议摘要 (ASCO/ESMO/AACR) 与公司更新未纳入本版。*")

out = os.path.join(BASE, "tce_report_20260803_final.md")
